...
public class GoToProcessorSourceFromS extends Command {
    private boolean valid;
    private Command pathFindingCommand;
    
    public Command GoToProcessorSideSourceFromS() {
        PathConstraints constraints = new PathConstraints(3, 3, 540, 720);

        PathPlannerPath path;
        try {
            path = PathPlannerPath.fromPathFile("ProcessorSideSourceSVision");
            valid = true;
