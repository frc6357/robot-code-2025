...
public class GoToProcessorSourceFromNE extends Command {
    private boolean valid;
    private Command pathFindingCommand;
    
    public Command GoToProcessorSideSourceFromNE() {
        PathConstraints constraints = new PathConstraints(3, 3, 540, 720);

        PathPlannerPath path;
        try {
            path = PathPlannerPath.fromPathFile("ProcessorSideSourceNEVision");
            valid = true;
