...
public class AlignToAnyReefTag extends Command {
    ...
    private Limelight findGoodLimelight() {
        for(Limelight ll : limelights) {
            if(m_vision.reefTargetCloseIgnoreAlliance(ll)) {
                return ll;
            }
        }
        return null;
    ...
    private Optional<Limelight> getTargetLimelight(AnyTarget t) {
        switch(t) {
            case ANY_LEFT:
                if(!m_vision.reefTargetCloseIgnoreAlliance(limelights[1])) {
                    break;
                }
                return Optional.of(limelights[1]);
            case ANY_RIGHT:
                if(!m_vision.reefTargetCloseIgnoreAlliance(limelights[0])) {
                    break;
                }
    ...
    private boolean setTargetLimelight(AnyTarget t) {
